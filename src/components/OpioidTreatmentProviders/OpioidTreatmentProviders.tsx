import Link from "next/link";
import type { OpioidTreatmentProvider } from "./OpioidTreatmentProvider.model";

export default function OpioidTreatmentProvidersComponent({ data }: { data: OpioidTreatmentProvider[] }) {
  return (
    <>
      {data?.map((opioidTreatmentProvider: OpioidTreatmentProvider, index: number) => {
        return (
          <div
            key={index}
            className="mb-2 w-[100%] rounded-lg bg-white shadow-lg"
          >
            <div className="p-2">
              <div className="flex flex-row justify-between">
                <div className="flex-auto">
                  <h5 className="text-md mb-2 w-[75%] font-medium text-violet-700 underline">
                    <Link
                      onClick={() =>
                        localStorage.setItem(
                          `${opioidTreatmentProvider?.id}`,
                          JSON.stringify(opioidTreatmentProvider)
                        )
                      }
                      href={`/opioidTreatmentProvider?opioid_treatment_provider_id=${
                        opioidTreatmentProvider?.id || ""
                      }`}
                    >
                      {opioidTreatmentProvider?.provider_name || "-"}
                    </Link>
                  </h5>
                  <div className="flex w-[75%] flex-row justify-between">
                    <h5 className="text-md mb-2 text-gray-900">
                      {opioidTreatmentProvider.city}, {opioidTreatmentProvider?.state.toUpperCase() || "-"}
                    </h5>
                    <p className="mb-1 text-base text-gray-700"> </p>
                  </div>
                  <div className="flex flex-row justify-between text-sm">
                    <p className="mb-1 text-xs text-violet-400">
                      United States
                    </p>
                    <div className="border-gray-300 text-gray-600"></div>
                  </div>
                </div>
                <div className="w-[25%]">
                  <div className="flex flex-col">
                    <p className="mb-1 text-right text-sm text-gray-600">
                        Opioid Treatment Provider ID: {opioidTreatmentProvider.id}
                      <br />
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        );
      })}
    </>
  );
}
