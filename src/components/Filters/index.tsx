...
export default function Filters({
  ...
  const filteredDocListToggle = () => {
    if (
      filterParams.doctorFilter ||
      filterParams.manufacturerFilter ||
        ...
        <div>
          <div className="filters flex w-full items-center">
            ...
            <div className="wrap-filters flex w-full items-center py-2">
              {data && (data?.doctors || data?.manufacturers) && (
                <select
                  onChange={(e) => {
              ...
              {data && data?.zipCodes && (
                ...
                >
                  <option value="">
                    {filterParams.zipCode == "" ? "Zip Code" : "Reset"}
                  </option>
              ...
              {data && data?.specialties && (
                ...
                >
                  <option value="">
                    {filterParams.specialty == "" ? "Specialty" : "Reset"}
                  </option>
                  {data?.specialties
                    .sort()
              ...
              {data && data?.products && (
                ...
                >
                  <option value="">
                    {filterParams.category == "" ? "Category" : "Reset"}
                  </option>
                  {data?.productTypes.map(
                    (
                      item: { type: string; category: string },
                      ...
                      <option key={index} value={item.category}>
                        {item.category === "NULL" ? "Misc" : item.category}
                      </option>
                    )
                  )}
                </select>
              ...
              {data && data?.payments && listData?.doctorNames && (
                <select
                  onChange={(e) => {
                  ...
                  value={filterParams.doctorFilter}
                  className="my-2 mx-1 w-[20%] cursor-pointer rounded-lg bg-violet-500 p-1 text-white hover:bg-violet-400 hover:text-violet-900"
                  name="state-filter"
                  id="state-filter"
                >
                  <option value={""}>
                    {filterParams.doctorFilter == "" ? "Doctor" : "Reset"}
                  </option>
                  {filteredDocListToggle()
                    ?.sort()
                    .map(
              ...
              {data && data?.payments && listData?.manufacturerNames && (
                <select
                  onChange={(e) => {
                    setFilterParams((prev: FilterParams) => {
                      return {
                  ...
                  value={filterParams.manufacturerFilter}
                  className="my-2 mx-1 w-[20%] cursor-pointer rounded-lg bg-violet-500 p-1 text-white hover:bg-violet-400 hover:text-violet-900"
                  name="state-filter"
                  id="state-filter"
                >
                  <option value="">
                    {filterParams.category == "" ? "Manufacturer" : "Reset"}
                  </option>
                  {filteredManuListToggle()?.map(
                    (item: { id: string; name: string }, index: number) => (
                      <option key={index} value={item.id}>
                        {item.name}
                      </option>
              ...
              {data && data?.payments && listData?.productNameList && (
                <select
                  onChange={(e) => {
                    setFilterParams((prev: FilterParams) => {
                      return {
                  ...
                  value={filterParams.productFilter}
                  className="my-2 mx-1 w-[20%] cursor-pointer rounded-lg bg-violet-500 p-1 text-white hover:bg-violet-400 hover:text-violet-900"
                  name="state-filter"
                  id="state-filter"
                >
                  <option value="">
                    {filterParams.productFilter == "" ? "Product" : "Reset"}
                  </option>
                  {filteredProdListToggle()?.map(
                    (item: { id: string; name: string }, index: number) => (
                      <option key={index} value={item.id}>
                        {item.name}
                      </option>
